...
const ZDGConnection = async () => {
   ...
   ZDGsock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      if (!msg.key.fromMe && jid !== 'status@broadcast' && !ZDGGroupCheck(jid)) {
         ...
         // mensagem de texto
         if (msg.message.conversation.toLowerCase() === 'iniciar') {
            ZDGSendMessage(jid, { text: 'Olá, ' + ZDGUsuario + '! Você está conversando com um *Robô*. Por favor digite seu *CPF*' })
         ...
         if (msg.message.conversation.length === 11) { //validar cpf com REGEX
            ZDGSendMessage(jid, { text: 'o CPF: *' + msg.message.conversation + '* Está correto?' })
               .then(result => {
         ...
         if (cpf && msg.message.conversation.toLocaleLowerCase() === "sim") {
