...
const ZDGConnection = async () => {
   ...
   const ZDGSendMessage = async (jid, msg) => {
      await ZDGsock.presenceSubscribe(jid) //envia presença
      await delay(2000)
   ...
   app.post('/zdg-message', [
      ...
      const numberDDI = jid.toString().substr(0, 2); //tive que alterar o jid para String
      const numberDDD = jid.toString().substr(2, 2);
      const numberUser = jid.toString().substr(-8, 8);
