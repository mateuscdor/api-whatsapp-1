...
async function syncMessages(limitMsg) {
  let dataFile = [];
  fs.writeFileSync("messages.json", JSON.stringify(dataFile));
  const chats = await client.getChats();
  for (const chat of chats) {
      const messages = await chat.fetchMessages({limit: limitMsg});
      for (const message of messages) {
        const messageFromMe = message.id.fromMe;
        const messageRemote = message.id.remote;
        const messageID= message.id.id;
        const messageBody = message.body;
        console.log(JSON.stringify("messageFromMe: " + messageFromMe + " | messageRemote: " + messageRemote +  " | messageID:" +  messageID + " | messageBody: " + messageBody));
        async function readWriteFileJson() {
          ...
          let newData = {
            ...
            await fsPromises.writeFile("messages.json", JSON.stringify(myObject), (err) => {
              // Error checking
              if (err) throw err;
...
app.post('/send-button', [
  ...
  const button = new Buttons(buttonBody,[{body:bt1},{body:bt2}],buttonTitle,buttonFooter);
...
app.post('/json', [
  ...
  const limitMsg = req.body.limite;

  syncMessages(limitMsg).then(response => {
    res.status(200).json({
      status: true,
      response: 'messages.json criado com sucesso'
    });
  }).catch(err => {
    res.status(500).json({
      status: false,
